...
更新に対して、宣言的な描画ルールを維持するようにUIが変更される
```

- `import` の書く順番、ちょっと拘ってみた（適切かどうかは自信ない）
  - 段落分けてみた
    - 一段落目: `react` や `react-redux` 等、基本的なライブラリ
    - 二段落目: `@material-ui` や `@storybook` 等、オプショナルなライブラリ(めちゃ感覚)
    - 三段落目: `core/`
    - 四段落目: `views/`
  - 段落内の順番も依存関係を意識してみた
    - `domain` と `usecase` は `domain` を先に書くなど。

- モジュール名は、一般的なnpmモジュールに合わせて、ハイフン区切りにした
...
- `domain` から実装すること
  - モデルを中心に、そのモデルに関するルールを記述すること
    ...
    - 無理な操作は行わない
    - とはいえ、backendに移譲する量が多いので難しいかもしれない。要検討。
